...
*/
channelRouter.post('/upsert', async (req, res) => {
    ...
    try {
        await prisma_1.default.requestChannel.upsert({
...
*/
channelRouter.post('/blacklist-add', async (req, res) => {
    ...
    try {
        const requestChannel = await prisma_1.default.requestChannel.findUnique({
            where: { owner_account_id: owner_account_id },
        ...
        await prisma_1.default.requestChannel.update({
            where: { owner_account_id: owner_account_id },
...
*/
channelRouter.post('/blacklist-remove', async (req, res) => {
    const { owner_account_id, user_id, token } = req.body;
    if (process.env.REQUIRE_TOKEN == "1") {
        const verify = (0, auth_1.verifyToken)(token);
        if (verify.err) {
...
*/
channelRouter.post('/add-level', async (req, res) => {
    ...
    try {
        ...
        const levelsArray = requestChannel.levels ?? [];
...
*/
channelRouter.post('/remove-level', async (req, res) => {
...
/*
    ...
    DESC: Get a page of the request channel list, ordered primarily by the channel being open and ordered secondarily by mod level, then by alphabetical order
          Token not required
*/
channelRouter.post('/get-channels-page', async (req, res) => {
    const { page } = req.body;
    if (page == null || page < 1) {
        res.status(400).json({
            success: false,
    ...
    const pageSize = 20;
    try {
        const channels = await prisma_1.default.requestChannel.findMany({
        ...
        const channelCount = await prisma_1.default.requestChannel.count();
...
/*
    ...
    DESC: Get all of the info of a channel
          Token not required
*/
channelRouter.post('/get-channel', async (req, res) => {
    ...
    try {
        const channel = await prisma_1.default.requestChannel.findUnique({
            where: { owner_account_id: owner_account_id }
        });
        res.status(200).json({
            success: true,
...
//# sourceMappingURL=requestchannel.js.map